...
function quantity(n,singular,plural){return n+' '+(n===1?singular:plural);}
function automaticDetail(key){var issues=issueStatus();if(key==='issues_resolved'){if(issues.pending===0)return issues.resolved?quantity(issues.resolved,'resolvida','resolvidas')+', nenhuma pendente':'Nenhuma pendência aberta';return quantity(issues.pending,'pendência','pendências');}if(key==='sources_verified'){var used=state.root?state.root.querySelectorAll('.verbum-revision-sources input[type="checkbox"]').length:0;return used?(itemValue(['sources_verified','','auto'])?'Todas conferidas':'Há fontes para conferir'):'Nenhuma fonte utilizada';}if(key==='completed')return itemValue(['completed','','auto'])?'Concluída':'Aguardando conclusão';return '';}
function groupSummaryHtml(){return'<div class="verbum-hom013-summary" aria-label="Resumo do progresso da Revisão">'+GROUP_ORDER.map(function(name){var p=groupProgress(name);return'<button type="button" data-hom013-jump="'+name+'" class="'+(state.mode===name?'is-active':'')+'"><span>'+esc(GROUPS[name].label)+'</span><strong>'+p.pct+'%</strong></button>';}).join('')+'</div>';}
function rowHtml(item){var key=item[0],label=item[1],kind=item[2],on=itemValue(item),disabled=false,detail='';if(kind==='auto'){disabled=true;detail=automaticDetail(key);}if(kind==='core'&&CORE_PREREQS[key]&&!prerequisitesReady(key)){disabled=true;detail='Conclua os critérios acima para liberar.';}var control=kind==='auto'?'<span class="verbum-hom013-auto-state" aria-hidden="true">'+(on?'✓':'•')+'</span>':'<input type="checkbox" '+(kind==='hom'?'data-hom013-flag="'+key+'"':'data-hom013-core="'+key+'"')+' '+(on?'checked':'')+' '+(disabled?'disabled':'')+'>';return'<label class="verbum-hom013-item '+(on?'is-complete':'')+' '+(kind==='auto'?'is-automatic':'')+'">'+control+'<span><b>'+esc(label)+'</b>'+(detail?'<small>'+esc(detail)+'</small>':'')+'</span>'+(kind==='auto'?'<em>Automático</em>':'')+'</label>';}
function panelHtml(){var overall=overallProgress(),group=GROUPS[state.mode],gp=groupProgress(state.mode);return'<div class="verbum-hom013-panel" data-hom013-panel><div class="verbum-hom013-overall"><div><span>Progresso desta Revisão</span><strong>'+overall.pct+'%</strong></div><div class="verbum-hom013-progress"><i style="width:'+overall.pct+'%"></i></div><small>'+overall.done+' de '+overall.total+' critérios concluídos</small></div>'+groupSummaryHtml()+'<div class="verbum-hom013-current"><div class="verbum-hom013-current-head"><div><span>Checklist atual</span><h4>'+esc(group.label)+'</h4></div><strong>'+gp.pct+'%</strong></div><p>'+esc(group.help)+'</p><div class="verbum-hom013-progress"><i style="width:'+gp.pct+'%"></i></div><div class="verbum-hom013-items">'+group.items.map(rowHtml).join('')+'</div></div>'+(state.mode==='final'?'<div class="verbum-hom013-final-note">A validação final reúne critérios gerais. Itens automáticos mudam conforme fontes, pendências e conclusão da Revisão.</div>':'')+'</div>';}
