...
const trashRuntime = await readFile(resolve(root, 'src/trash-runtime.js'), 'utf8');
...
requireAll(technicalRuntime, ["internalProject.type='hidden'",'projectLabel.remove()','ambiente da obra'], 'Cadastro de obra sem projeto visível HOM-046');
const staticUpdateRuntime = await readFile(resolve(root, 'src/static-runtime.js'), 'utf8');
const officialDashboardUpdate = await readFile(resolve(root, 'src/dashboard-official-runtime.js'), 'utf8');
requireAll(staticUpdateRuntime, ['verbum:library-updated','CustomEvent'], 'Atualização da biblioteca HOM-047');
requireAll(officialDashboardUpdate, ['verbum:library-updated','load(root)'], 'Atualização automática do Painel HOM-047');
