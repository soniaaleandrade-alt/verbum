...
const requiredFiles = [
  ...
  'src/styles/verbum.css','src/styles/library.css','src/styles/workspace.css','src/styles/identification.css','src/styles/project-stage.css','src/styles/planning-stage.css','src/styles/development-stage.css','src/styles/chapter-preparation.css','src/styles/chapter-research.css','src/styles/chapter-workflow.css','src/styles/technical.css','src/styles/dashboard-official.css','src/styles/minhas-obras.css','src/styles/auth-profile.css','src/styles/profile-polish.css',
  'src/auth-profile-runtime.js','src/static-runtime.js','src/workspace-mobile-runtime.js','src/identification-runtime.js','src/project-stage-runtime.js','src/planning-stage-runtime.js','src/development-stage-runtime.js','src/chapter-workflow-runtime.js','src/chapter-preparation-runtime.js','src/chapter-research-runtime.js','src/technical-runtime.js','src/dashboard-official-runtime.js','src/sidebar-profile-runtime.js','src/minhas-obras-runtime.js','src/profile-polish-runtime.js','src/vite-env.d.ts',
];
...
const development = await readFile(resolve(root, 'src/components/DevelopmentStage.tsx'), 'utf8'); requireAll(development, ['Desenvolvimento da Obra','ChapterPreparationStage','ChapterResearchStage','chapterViewStage','Preparação','Pesquisa','Redação','Revisão','Abrir capítulo'], 'DevelopmentStage');
...
const preparationRuntime = await readFile(resolve(root, 'src/chapter-preparation-runtime.js'), 'utf8'); requireAll(preparationRuntime, ['/preparation','/preparation/complete','Concluir Preparação','Fontes que deverão ser pesquisadas','source_categories'], 'Chapter preparation runtime');
const research = await readFile(resolve(root, 'src/components/ChapterResearchStage.tsx'), 'utf8'); requireAll(research, ['Direção da Pesquisa','Central de Pesquisa','Banco de Ideias do capítulo','Selecionados para Redação','Concluir Pesquisa','completeChapterResearch','createChapterResearchSource'], 'ChapterResearchStage');
const researchRuntime = await readFile(resolve(root, 'src/chapter-research-runtime.js'), 'utf8'); requireAll(researchRuntime, ['/research','/research/sources','/research/complete','Direção da Pesquisa','Central de Pesquisa','Banco de Ideias do capítulo','Concluir Pesquisa','selected_for_writing'], 'Chapter research runtime');
const workflowRuntime = await readFile(resolve(root, 'src/chapter-workflow-runtime.js'), 'utf8'); requireAll(workflowRuntime, ['verbum_chapter_stage','Preparação','Pesquisa','is-selected'], 'Chapter workflow runtime');
...
const researchCss = await readFile(resolve(root, 'src/styles/chapter-research.css'), 'utf8'); requireAll(researchCss, ['.verbum-research-layout','.verbum-research-direction','.verbum-research-source-card','.verbum-research-progress','.verbum-research-ideas','@media'], 'Chapter research CSS');
const libraryService = await readFile(resolve(root, 'src/services/library-service.ts'), 'utf8'); requireAll(libraryService, ['getChapterResearch','saveChapterResearchState','createChapterResearchSource','updateChapterResearchSource','deleteChapterResearchSource','completeChapterResearch'], 'Library service');
...
const buildJs = await readFile(resolve(repoRoot, 'build/verbum-app.js'), 'utf8'); requireAll(buildJs, ['development-stage-runtime.js','chapter-workflow-runtime.js','chapter-preparation-runtime.js','chapter-research-runtime.js','script.async=false'], 'Static JS build');
const buildCss = await readFile(resolve(repoRoot, 'build/verbum-app.css'), 'utf8'); requireAll(buildCss, ['development-stage.css','chapter-preparation.css','chapter-research.css','chapter-workflow.css','profile-polish.css'], 'Static CSS build');
...
console.log('Frontend Sprint 10 Pesquisa do Capítulo checks passed');
