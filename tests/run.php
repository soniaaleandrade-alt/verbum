...
test('config exposes core defaults and production mode', function (): void {
    ...
    assert_same('2.1.0', $config->get('version'));
...
test('health endpoint returns ok and version', function (): void {
    ...
    assert_same('2.1.0', $data['data']['version']);
});
...
test('shortcode enqueues workflow through audit assets', function (): void {
    global $verbum_test_enqueued, $verbum_test_json_request;
    $verbum_test_enqueued = [];
    $verbum_test_json_request = false;
    assert_same('<div class="verbum-app" data-verbum-app></div>', (new FrontendAssets())->shortcode());
    $serialized = json_encode($verbum_test_enqueued);
    foreach (['planning-stage.css','development-stage.css','chapter-preparation.css','chapter-research.css','chapter-writing.css','chapter-revision.css','general-review.css','work-versions.css','work-audit.css'] as $asset) {
...
test('Sprint 15 REST routes are registered', function (): void {
    ...
    foreach ([
        ...
        'verbum/v1/books/(?P<id>\\d+)/general-review','verbum/v1/books/(?P<id>\\d+)/general-review/reading','verbum/v1/books/(?P<id>\\d+)/general-review/issues','verbum/v1/books/(?P<id>\\d+)/general-review/issues/(?P<issue_id>[a-zA-Z0-9_-]+)','verbum/v1/books/(?P<id>\\d+)/general-review/complete','verbum/v1/books/(?P<id>\\d+)/general-review/assist',
