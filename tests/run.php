...
test('config exposes core defaults and production mode', function (): void {
    ...
    assert_same('2.5.5', $config->get('version'));
...
test('health endpoint returns ok and version', function (): void {
    ...
    assert_same(200, $response->get_status()); assert_same(true, $data['success']); assert_same('ok', $data['data']['status']); assert_same('2.5.5', $data['data']['version']);
});
