...
test('config exposes core defaults and production mode', function (): void {
    ...
    assert_same('1.9.0', $config->get('version'));
...
test('health endpoint returns ok and version', function (): void {
    ...
    assert_same('1.9.0', $data['data']['version']);
});
...
test('shortcode enqueues chapter workflow through general review assets', function (): void {
    global $verbum_test_enqueued, $verbum_test_json_request;
    $verbum_test_enqueued = [];
    $verbum_test_json_request = false;
    assert_same('<div class="verbum-app" data-verbum-app></div>', (new FrontendAssets())->shortcode());
    $serialized = json_encode($verbum_test_enqueued);
    foreach (['planning-stage.css','development-stage.css','chapter-preparation.css','chapter-research.css','chapter-writing.css','chapter-revision.css','general-review.css'] as $asset) {
...
test('Sprint 13 REST routes are registered', function (): void {
